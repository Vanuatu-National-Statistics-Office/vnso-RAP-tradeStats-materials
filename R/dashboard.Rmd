...
### Trades in Goods and Services by Year
...
```{r animated graphic for top 10 countries by year for imports, eval=FALSE}
...
```{r animated graphic for top 10 countries by year for exports, eval=FALSE}
exp %>%
  mutate_if(is.factor, as.character)%>%
...
importHistorysummarySubset <- reactive({
  importHistorysummary %>%
          filter(HS==input$MJTN) %>%
          group_by(Year) %>%
           summarise(Value = sum(Value_Sum), .groups="keep") %>%
  ungroup()
...
```{r}
renderPlot({
 importHistorysummarySubset() %>% 
  plot_ly(aes_string(x=Year,y=Value_Sum,fill=Value_Sum)) +
  geom_line() +
  theme(panel.background = element_rect(fill = '#cccccc', colour = 'red'))
})
```
...
------------------------------------------------------------------
```{r}
selectInput(inputId = "MJ",label = "Group By",choices = CEChoices)
selectInput(inputId = "Metric_two",label = "Metric",choices = names(select_if(countryExport,is.numeric)))
```
