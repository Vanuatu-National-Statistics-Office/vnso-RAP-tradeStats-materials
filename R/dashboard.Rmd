...
>>>>>>> 15a720a08ee49ca902a479b02f67af98de9c96ce
...
renderPlot({
 updateData_import_commodity() %>% 
  ggplot(aes_string(x=Year,y=input$MJTN,fill=Value_Sum)) +
  geom_col() +
  theme(panel.background = element_rect(fill = '#cccccc', colour = 'red'))
})
```
...
------------------------------------------------------------------
```{r}
selectInput(inputId = "exportCountry",label = "Metric",choices = names(select_if(countryExport,is.numeric)))
```
...
renderPlot({
 updateData_import_country() %>% 
  ggplot(aes_string(x=Year,y=input$exportCountry,fill=Value_Sum)) +
  geom_col() +
  theme(panel.background = element_rect(fill = '#cccccc', colour = 'red'))
})
```
