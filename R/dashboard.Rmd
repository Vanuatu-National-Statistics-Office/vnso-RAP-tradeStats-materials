...
params:
    useSQL: FALSE # Select whether to load data from SQL server
    startDate: "2019-01-01" # Start date for latest month (YYYY-dd-mm)
    endDate: "2019-01-31" # End date for latest month (YYYY-dd-mm)
---
...
repository <- dirname(dirname(rstudioapi::getSourceEditorContext()$path))

# Define color palette
myColors <- c("blue", "#FFC125", "darkgreen", "darkorange")

# Convert the latest month dates to dates
params$startDate <- as.Date(params$startDate)
params$endDate <- as.Date(params$endDate)
```
...
imp <- read.csv(file.path("data", "secure", "country_year_import.csv")) # I don't have access to this file
exp <- read.csv(file.path("data", "secure", "country_year_export.csv")) # I don't have access to this file
...
if(params$useSQL){
  ...
  countries <- fetch(country, n=-1)$CNTY_DESCR
}else{
  countries <- read.csv("https://raw.githubusercontent.com/umpirsky/country-list/master/data/en_US/country.csv")[, 2]
}

selectInput(inputId = "selectedCountry", label = "Select Country", choices = countries$CNTY_DESCR)
...
if(params$useSQL){
  ...
  pExport <- principleCOmmodities$Principle.Exports
}
