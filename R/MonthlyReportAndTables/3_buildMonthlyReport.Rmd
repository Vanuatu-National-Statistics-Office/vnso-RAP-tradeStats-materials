...
processedTradeStats <- read.csv(file.path(secureDataFolder, "OUT_PROC_ASY_ProcessedRawData_30-09-21.csv"))
...
historicalExports$Month <- month.name[historicalExports$Month]
...
ggplot(pacificCountryTotalsLong, aes(x=Country, y= (Value/1000000),  fill=Type)) +
  ...
  theme_minimal() + 
  labs(title="Trade Balance of South Pacific Islands", y = "International Merchandise Trade (millions)") +
...
ggplot(data = tradeBalanceByCountryOther[c(1:5, (nRows-4):nRows), ], 
  ...
  labs(title="Trade Balance of New Emerging Markets", y = "Trade Balance (millions)") +
  theme_minimal() +
  theme(legend.title=element_blank(),
        axis.text.x = element_text(angle = 35, vjust = 1.1, hjust=1, size = 5),
        axis.title.y = element_text(size = 9),
...
ggplot(data = totalMajorExports, aes(x = Year, y = value, group = factor(name_shortened))) +
  ...
  labs(title="Principle Exports", y = "Total export value (millions)") +
  theme_minimal() +
...
exportsOtherandNA %>%
  ...
  ggplot(aes(x = reorder(Goods.Comm..Dsc., -total), y = (total/1000000))) +
    geom_bar(stat = "identity", color="black", fill= "lightsteelblue1", position=position_dodge()) +
      labs(title="Top 5 New Major Exports", y = "Total export value (millions)") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 35, vjust = 1.1, hjust=1, size = 5),
          axis.title.y = element_text(size = 9),
          axis.title.x = element_blank())
...
ggplot(data = totalMajorImports, aes(x = Year, y = value, group = factor(name_shortened))) +
  geom_line(aes(color = factor(name_shortened)), size = 1.5, alpha = 0.75) +
  labs(title="Principle Imports", y = "Total import value (millions)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 35, vjust = 1.1, hjust=1, size = 5),
        axis.title.y = element_text(size = 9),
        axis.title.x = element_blank(),
...
importsOther %>%
  ...
  ggplot(aes(x = reorder(Goods.Comm..Dsc., -total), y = (total/1000000))) +
    geom_bar(stat = "identity", color="black", fill= "lightsteelblue1", position=position_dodge()) +
  labs(title="Top 5 New Major Imports", y = "Total import value (millions)") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 35, vjust = 1.1, hjust=1, size = 5),
          axis.title.y = element_text(size = 9),
          axis.title.x = element_blank())
...
```{r health imports through time, fig.height = 1.8, eval= FALSE}
...
```{r food and products imports through time, fig.height=1.75, eval= FALSE}
