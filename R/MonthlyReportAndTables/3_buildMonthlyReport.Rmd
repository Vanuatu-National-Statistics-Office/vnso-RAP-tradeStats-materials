...
tradeAgreementCurrentMonthlyImports <- processedTradeStats[is.na(processedTradeStats$PRF) == FALSE & processedTradeStats$PRF %in% c("MSG", "PICTA"), ]
tradeAgreementCurrentMonthlyImportValue<- sum(tradeAgreementCurrentMonthlyImports$Stat..Value)
...
tradeStatsForAllMSGCountryExports <- msgCommoditiesExcludeMerged[msgCommoditiesExcludeMerged$CP4 == 1000 & msgCommoditiesExcludeMerged$EXPORT.COUNTRY %in% c("FIJI", "NEW CALEDONIA", "PAPUA NEW GUINEA", "SOLOMON ISLANDS"), ]
tradeAgreementCurrentMonthlyExports <- tradeStatsForAllMSGCountryExports[is.na(tradeStatsForAllMSGCountryExports$Not.Included.in.MSG) == TRUE, ]
tradeAgreementCurrentMonthlyExportValue<- sum(tradeStatsForMSGExports$Stat..Value)

# Proxy indicator calculated for NSDP indicator ECO 1.5.2 by combining imports and proxy exports
proxyIndicator1.5.2CurrentMonth<- tradeAgreementCurrentMonthlyImportValue + tradeAgreementCurrentMonthlyExportValue

# Insert values into table
proxyIndicatorCurrentMonth1.5.2DataFrame <- data.frame("Trade by Trade Agreement Monthly Value"= proxyIndicator1.5.2CurrentMonth)
...
# Build the table
majorPartnerCountryCurrentMonthExportValue <- buildRawSummaryTable(processedTradeStats, codesCP4, categoryColumn, columnCategories)
totalMajorPartnerCountryCurrentMonthExportValue<- majorPartnerCountryCurrentMonthExportValue$Total
...
# Build the table
majorPartnerCountryCurrentMonthImportValue <- buildRawSummaryTable(processedTradeStats, codesCP4, categoryColumn, columnCategories)

# Indicator calculated for NSDP indicator ECO 1.5.3 by subtracting total imports from exports 
monthlyBalanceOfTradeByMajorPartnerCountries <- majorPartnerCountryCurrentMonthExportValue - majorPartnerCountryCurrentMonthImportValue
indicator1.5.3CurrentMonthCalculated<- monthlyBalanceOfTradeByMajorPartnerCountries$Total

# Insert values into table
indicator1.5.3CurrentMonthDataFrame <- data.frame("Current Month Balance of Trade by Major Partner Countries"= indicator1.5.3CurrentMonthCalculated)


## ECO 1.6.2 Total estimated value of trade agreement as a proportion of GDP
## Proxy calculated: estimated value of trade agreements and use of 2018 GDP   
...
proxyIndicator1.6.2CurrentMonth<- (proxyIndicator1.5.2CurrentMonth/mostRecentGrossDomesticProduct)*100

# Insert values into table as a percentage to 2 decimal places
proxyIndicator1.6.2CurrentMonthRounded<- round(proxyIndicator1.6.2CurrentMonth, digits = 2)
proxyIndictaor1.6.2CurrentMonthDataFrame <- data.frame("Value of Trade Agreement as Proportion of GDP for Current Month"= proxyIndicator1.6.2CurrentMonthRounded)
...
# Build the table
majorExportCurrentMonthCommoditiesValue <- buildRawSummaryTable(processedTradeStats, codesCP4, categoryColumn, columnCategories)

# Proxy indicator for NSDP ECO 1.7.1
proxyIndicator1.7.1CurrentMonthValue<- majorExportCurrentMonthCommoditiesValue$Total
proxyIndictaor1.7.1DataFrame <- data.frame("Level of production of major commodities to include Cocoa and Copra for Current Month"= proxyIndicator1.7.1CurrentMonthValue)

## ECO 4.3.2 Ratio of processed export commodities (including coconut, kava, cocoa, coffee) to raw exports

# Read in the data from the open folder of the repository for processed & raw Commodities (coconut, kava, cocoa and coffee)
...
processedRawCommoditiesMergedWithClassifications <- merge(processedTradeStats, processedRawCommodities, by="HS.Code", all.x=TRUE)

# Create data-frame for all raw and processed export commodities for current month
processedRawCommoditiesFiltered <- processedRawCommoditiesMergedWithClassifications[is.na(processedRawCommoditiesMergedWithClassifications$Source) == FALSE, ]
processedRawCommoditiesExported<- processedRawCommoditiesFiltered[processedRawCommoditiesFiltered$CP4 %in% c(1000, 3071), ]
...
indicator4.3.2<- (110333195/35106247)
...
tradeStatsMSGBaselineExports<- tradeStatsMSGExports2016[tradeStatsMSGExports2016$Procedure == 1000 &     tradeStatsMSGExports2016$CTY_Dest %in% c("FJ", "NC", "PG", "SB"), ]
tradeStatsMSGBaselineExports$HS<- sapply(tradeStatsMSGBaselineExports$HS, FUN=padWithZeros, "HS")
tradeStatsMSGBaselineExports$HS.Code_6 <- extractCodeSubset(tradeStatsMSGBaselineExports$HS, nDigits=6)
msgAgreementHistoricalCommoditiesExcludeMerged <- merge(tradeStatsMSGBaselineExports, msgAgreementCommoditiesExclude, by="HS.Code_6", all.x=TRUE)
msgHistoricalCommodities <- msgAgreementHistoricalCommoditiesExcludeMerged[is.na(msgAgreementHistoricalCommoditiesExcludeMerged$Not.Included.in.MSG) == TRUE, ]

# Calculate value of Trade Agreements by month for baseline year
msgHistoricalCommodities$Value <- as.numeric(gsub(",", "", msgHistoricalCommodities$Value))# Convert the statistical value to numeric
totalMonthlyValueTradeAgreementsExports <- msgHistoricalCommodities %>%
  group_by(Month) %>%
  summarise(total = sum(Value))

# Calculate value of Trade Agreements for current month from baseline year (need to change from hard coding)
...
proxyIndicator1.5.2TargetCurrentMonth<- ((tradeAgreementCurrentMonthlyExportValue - tradeAgreementCurrentMonthBaselineValue)/tradeAgreementCurrentMonthBaselineValue)*100 # needs to be 15% to meet Target

# Insert values into table as a percentage to 2 decimal places
proxyIndicator1.5.2TargetCurrentMonthRounded<- round(proxyIndicator1.5.2TargetCurrentMonth, digits = 2)
proxyIndictaor1.6.2CurrentMonthDataFrame <- data.frame("Percentage Chnage in exports for Current month compared to Baseline)"= proxyIndicator1.5.2TargetCurrentMonthRounded)
...
historicalDataMonthlyValueMajorPartnerCountryExports <- historicalTradeStatsExportsMajorPartnerCountries %>%
  group_by(Month, EXPORT.COUNTRY) %>%
  summarise(total = sum(Value))

totalMonthlyValueMajorPartnerCountryExports <- historicalDataMonthlyValueMajorPartnerCountryExports %>%
  group_by(Month) %>%
  summarise(total = sum(total))

# Calculate current monthly value
historicalDataCurrentMonthlyValueMajorPartnerCountryExports<- totalMonthlyValueMajorPartnerCountryExports[totalMonthlyValueMajorPartnerCountryExports$Month == 1, ]
historicalMonthlyExportValueproxyIndicator1.5.3Target<- historicalDataCurrentMonthlyValueMajorPartnerCountryExports$total

# Calculate ECO 1.5.3 Balance of Trade by major partner countries, Target 2020 exports increase by 20%
proxyIndicator1.5.3TargetCurrentMonth<- ((totalMajorPartnerCountryCurrentMonthExportValue - historicalMonthlyExportValueproxyIndicator1.5.3Target)/historicalMonthlyExportValueproxyIndicator1.5.3Target)*100 # needs to reach 20% Target

# Insert values into table as a percentage to 2 decimal places
proxyIndicator1.5.3TargetCurrentMonthRounded<- round(proxyIndicator1.5.3TargetCurrentMonth, digits = 2)
proxyIndicator1.5.3TargetCurrentMonthDataFrame <- data.frame("Percentage Change in exports for Current month compared to Baseline)"= proxyIndicator1.5.3TargetCurrentMonthRounded)
...
# Calculate value of Trade Agreements by month for baseline year
msgHistoricalCommodities$Value <- as.numeric(gsub(",", "", msgHistoricalCommodities$Value))# Convert the statistical value to numeric
totalMonthlyValueTradeAgreementsHistoricalExports <- msgHistoricalCommodities %>%
  group_by(Month) %>%
  summarise(total = sum(Value))

# Calculate value of Trade Agreements of GDP for baseline year for current month
grossDomesticProduct <- timeSeriesGrossDomesticProduct[nrow(timeSeriesGrossDomesticProduct), "Value"] #identify 2016 GDP
grossDomesticProduct2016<- timeSeriesGrossDomesticProduct[timeSeriesGrossDomesticProduct$Year == 2016, "Value" ]
trade
tradeAgreementCurrentMonthBaselineValue <- sum(totalMonthlyValueTradeAgreementsExports[totalMonthlyValueTradeAgreementsExports$Month == 1, "total"], na.rm= TRUE)
proxyIndicator1.6.2CurrentMonthBaseline<- (tradeAgreementCurrentMonthBaselineValue/grossDomesticProduct2016)*100

# Calculate ECO 1.6.2 Total estimated value of trade agreement as proportion of GDP, Target sets an increase by 15% by 2030
proxyIndicator1.6.2TargetCurrentMonth<- ((proxyIndicator1.6.2CurrentMonth - proxyIndicator1.6.2CurrentMonthBaseline)/proxyIndicator1.6.2CurrentMonthBaseline)*100 # needs to be 15% to meet Target

# Insert values into table as a percentage to 2 decimal places
proxyIndicator1.6.2TargetCurrentMonthRounded<- round(proxyIndicator1.6.2TargetCurrentMonth, digits = 2)
proxyIndicator1.6.2TargetCurrentMonthDataFrame <- data.frame("Percentage Change in total estimated value of trade agreement as proportion of GDP for Current month compared to Baseline)"= proxyIndicator1.6.2TargetCurrentMonthRounded)
...
# Read in the data from the secure folder of the repository. Need to calculate Baseline indicators. NSDP launched in 2017, use 2016 data for baseline 
tradeStatsExports2016<- historicalExports[historicalExports$Year == 2016, ]
tradeStatsExports2016$HS <- sapply(tradeStatsExports2016$HS, FUN=padWithZeros, "HS")

# Merge Principle Commodity Classifications of Exports with historical data
...
colnames(principleCommodityClassification)[1] <- "HS"
principleCommodityClassification$HS <- sapply(principleCommodityClassification$HS, FUN=padWithZeros, "HS")
historicalCommoditiesMergedWithClassifications <- merge(tradeStatsExports2016, principleCommodityClassification, by="HS", all.x=TRUE)
...
# Create data-frame for current month within historical data
historicalCommodities2016<- historicalCommoditiesMergedWithClassifications[historicalCommoditiesMergedWithClassifications$Month == 1, ]
...
# Build the table
historicalExportCurrentMonthCommoditiesValue <- buildRawSummaryTable(historicalCommodities2016, codesCP4, categoryColumn, columnCategories)
totalHistoricalExportCurrentMonthCommoditiesValue<- historicalExportCurrentMonthCommoditiesValue$Total

# Calculate ECO 1.7.1 Level of production of major commodities to include cocoa and copra- 2030 increase 10-15% of the proportion
targetValue<- ((proxyIndicator1.7.1CurrentMonthValue - totalHistoricalExportCurrentMonthCommoditiesValue)/totalHistoricalExportCurrentMonthCommoditiesValue)*100
...
columnCategories <- list(
  ...
  "Copra"=c("Copra"),
  "Kava"=c("Kava"))
majorExportsCurrentMonth <- buildRawSummaryTable(processedTradeStats, codesCP4, categoryColumn, columnCategories)

# Take data-frame and add to historical "majorMonthlyExports" 
...
totalCommodityValueByPrincipleExportType <- processedTradeStats %>%
  ...
  summarise(total = sum(Stat..Value), 
            ...
            principleImportCategory = Principle.Imports[1])
...
# Select top 5 new exports
top5OtherExports<- exportsOtherandNA %>% 
  select()
...
# Convert values
majorMonthlyImports$Year <- as.factor(majorMonthlyExports$Year)
majorMonthlyImports$Month <- as.factor(majorMonthlyExports$Month)
majorMonthlyImports$Alcoholic.Drinks <- as.numeric(majorMonthlyImports$Alcoholic.Drinks)
majorMonthlyImports$Articles.Iron.and.Steel <- as.numeric(majorMonthlyImports$Articles.Iron.and.Steel)
majorMonthlyImports$Medicaments <- as.numeric(majorMonthlyImports$Medicaments)
majorMonthlyImports$Petroleum.Oils.and.oils.obtained.from.bituminous.minerals <- as.numeric(majorMonthlyImports$Petroleum.Oils.and.oils.obtained.from.bituminous.minerals)
majorMonthlyImports$Rice <- as.numeric(majorMonthlyImports$Rice)
...
# Take data-frame and add to historical "majorMonthlyImports"
...
# Select only exports falling into the OTHER or NA category
importsOtherandNA <- orderedPrincipleImportType %>%
  filter(principleImportCategory %in% c("Other Imports"))

# Select top 5 new imports
top5OtherImports<- importsOtherandNA %>% 
  select()
...
# Imports of Products related to Health
...
# Merge unhealthy commodity classifications with cleaned data
productsRelatedHealth$HS.Code <- sapply(productsRelatedHealth$HS.Code, FUN=padWithZeros, "HS")
productsRelatedHealthMergedWithClassifications <- merge(processedTradeStats, productsRelatedHealth, by="HS.Code", all.x=TRUE)

# Create data-frame for all unhealthy imports for current month
unhealthlyCommodities <- productsRelatedHealthMergedWithClassifications[is.na(productsRelatedHealthMergedWithClassifications$Unhealthy.Focus.Food.Category) == FALSE, ]
...
# Create data-frame for all unhealthy imports from historical data 
historicalImportsFile <- file.path(secureDataFolder, "imports_HS_summaryStats_02-10-20.csv")
historicalImports<- read.csv(historicalImportsFile)
historicalImportsTenYears<- historicalImports[historicalImports$Year %in% c("2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019", "2020"), ]
colnames(historicalImportsTenYears)[6] <- "HS.Code"
historicalImportsTenYears$HS.Code <- sapply(historicalImportsTenYears$HS.Code, FUN=padWithZeros, "HS")
historicalHealthMergedWithClassifications <- merge(historicalImportsTenYears, productsRelatedHealth, by="HS.Code", all.x=TRUE)
unhealthlyHistoricalCommodities <- historicalHealthMergedWithClassifications[is.na(historicalHealthMergedWithClassifications$Unhealthy.Focus.Food.Category) == FALSE, ]

# Calculate value of Food Sub-Category for historical data
foodSubCategoryHistoricalValue <- unhealthlyHistoricalCommodities %>%
  group_by(Year, Month, Food.sub.category) %>%
  summarise(total = sum(Value))

# Calculate value of Food Sub-Category current month value for historical data
foodSubCategoryHistoricalMonthlyValue <- foodSubCategoryHistoricalValue %>%
  group_by(Year, Month, Food.sub.category) %>%
    filter(Month == 1) 
...
# Create a line graph
ggplot(historalAndCurrentMonthFood, aes(x=Year)) + 
  geom_line(aes(y = ), color = "darkred") + 
  geom_line(aes(y = ), color="steelblue", linetype="twodash")

# Divide monthly values by actual and estimated population data from the Census




### Code below not working not sure why
# Calculate value of unhealthy commodities for current month
codesCP4 <- c(4000, 4071, 7100)
categoryColumn <- "Food sub-category"
columnCategories <- list(
  "Processed Meats"=c("Processed Meats"),
  "Canned Meats"=c("Canned Meats"),
  "Cordial"=c("Cordial"),
  "Soft Drinks"=c("Soft Drinks"),
  "Electrolyte drinks"=c("Electrolyte drinks"),
  "Sugar"=c("Sugar"),
  "Crisps and snacks"=c("Crisps and snacks"),
  "Noodles"=c("Noodles"),
  "Confectionary"=c("Confectionary"),
  "Bakery products"=c("Bakery products")
  )
importsUnhealthyFoodCurrentMonth <- buildRawSummaryTable(unhealthlyCommodities, codesCP4, categoryColumn, columnCategories)
