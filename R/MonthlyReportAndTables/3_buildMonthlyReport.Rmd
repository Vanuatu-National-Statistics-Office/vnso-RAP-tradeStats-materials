...
processedTradeStats <- read.csv(file.path(secureDataFolder, "OUT_PROC_ASY_ProcessedRawData_31-12-24.csv"))
...
# Load DARD Classifications table
...
# Read in the data for monthly major exports values
...
# Read in the data for monthly major imports values
...
# Load the NCD classifications table
...
# Read in the data for NCD monthly unhealthy imports values
...
# Load the imports substitution (chicken, fish) classification table
...
# Read in the imports substitution (chicken, fish) monthly values
...
# Read in the Gross Domestic Product values 
...
importOnlyDataFrame <- importSubstitutionMerged[importSubstitutionMerged$CP4 %in% c(4000,  7100), ]
...
indicatorENV1.3.1<- round((sum(importSubstitutionDataFrame$Stat..Value)/1000000))
...
indicatorECO1.5.2<- round(((importTradeAgreementValue + exportTradeAgreementValue)/1000000))
...
indicatorECO1.5.3 <- round(((totalMajorPartnerCountryExportValue - totalMajorPartnerCountryImportValue)/1000000))
...
indicatorECO1.6.2<- round(tradeAgreementGDPProportion)
...
indicatorECO1.7.1<- round(((majorExportCommodities$Total)/1000000))
...
# Insert values into table as a percentage to 2 decimal places
indicatorECO4.3.2<- round(processedRawProportion$total)
...
nsdpIndicators$`Indicator Value` <- round(nsdpIndicators$`Indicator Value`)
...
# Convert the dataset to a long format to facilitate sorting
data_long <- totalPacificCountryExports %>%
  pivot_longer(
    cols = -Total,
    names_to = "Country",
    values_to = "Exports"
...
# Remove rows where export values are 0
data_long_2 <- data %>%
  filter(Exports != 0)

# Sort the data by Exports in descending order and select the top 5
top_five_exports <- data_long_2 %>%
  arrange(desc(Exports)) %>%
  head(5)

# Convert the top five data back to wide format
top_five_wide <- top_five_exports %>%
...
write.csv(pacificCountryTotals, "pacificTradeBalance.csv")
...
processedTradeStats <- processedTradeStats %>%
  mutate(CommodityType = case_when(CP4 %in% c(4000,  7100) ~ "IMPORT",
...
importTradeAgreement <- processedTradeStats[is.na(processedTradeStats$PRF) == FALSE & processedTradeStats$PRF %in% c("MSG"), ]
exportTradeAgreement<- allMSGCountryExports[is.na(allMSGCountryExports$Not.Included.in.MSG) == TRUE, ]
write.csv(exportTradeAgreement, "tradeByTradeAgreement.csv")
...
# Divide by 1,000,000
tradeByTradeAgreement[, -1] <- round(tradeByTradeAgreement[, -1] / 1000000)
...
write.csv(exportsOtherandNA, "newMajorExports.csv", row.names = FALSE)
...
totalMajorImports$Petroleum.Oils.and.oils.obtained.from.bituminous.minerals <- as.numeric(totalMajorImports$Petroleum.Oils.and.oils.obtained.from.bituminous.minerals)
# Convert the table to long format
...
write.csv(importsOther, "newMajorImports.csv", row.names = FALSE)
...
unhealthyImportCommodities<- unhealthlyCommodities[unhealthlyCommodities$CP4 %in% c(4000,  7100), ]

write.csv(unhealthyImportCommodities, "unhealthyImports.csv", row.names = FALSE)
...
subsetHistoralAndCurrentMonthFood<- historalAndCurrentMonthFood[historalAndCurrentMonthFood$Food.sub.category %in% c("Bakery products", "Confectionary", "Cordial", "Noodles", "Processed Meats"), ]
...
write.csv(mergedImportSubstitutions, "importSubstitution.csv", row.names = FALSE)
