...
## Proxy calculated: Calculate value of all international trade registered under a trade agreement- only imports registered, exports estimated (all exports to MSG countries included)

# Calculate value of imports that are from countries that have signed the MSG and PICTA trade agreements
tradeAgreementImports <- processedTradeStats[is.na(processedTradeStats$PRF) == FALSE & processedTradeStats$PRF %in% c("MSG", "PICTA"), ]
tradeAgreementImportsValue<- sum(tradeAgreementImports$Stat..Value)

# Calculate value of exports that are destined to countries that have signed the MSG trade agreement 
...
msgTradeAgreementProxyValueSeparated<- msgTradeAgreementProxyDataFrame <- data.frame("MSGImportValue"= msgImportValue, "MSGExportEstimateValue"= msgProxyExportValue)
...
aggregatedTotalPrincipleExports<- sum(totalPrincipleExports$Total)
...
percentageChangeJanuaryExports<- (msgTradeAgreementProxyValue/tradeAgreementJanuary2019)*100 # needs to be 120% to meet Target
