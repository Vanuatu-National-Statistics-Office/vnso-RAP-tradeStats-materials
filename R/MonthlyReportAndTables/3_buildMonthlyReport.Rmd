...
nsdpIndicators$`Target (%)` <- round(nsdpIndicators$`Target (%)`, digits = 2)
...
ggplot(pacificCountryTotalsLong, aes(x=Country, y= (Value/1000000),  fill=Type)) +
  ...
  theme_minimal() + 
  labs(title="Trade Balance of South Pacific Islands", y = "Vatu (millions)") +
...
ggplot(data = tradeBalanceByCountryOther[c(1:5, (nRows-4):nRows), ], 
  ...
  labs(title="Trade Balance of New Emerging Markets", y = "Vatu (millions)") +
  theme_minimal() +
  theme(legend.title=element_blank(),
        axis.text.x = element_text(angle = 35, vjust = 1.1, hjust=1, size = 5),
        axis.title.y = element_text(size = 9),
...
ggplot(data = totalMajorExports, aes(x = Year, y = value, group = factor(name_shortened))) +
  ...
  labs(title="Principle Exports", y = "Vatu (millions)") +
  theme_minimal() +
...
exportsOtherandNA %>%
  ...
  ggplot(aes(x = reorder(Goods.Comm..Dsc., -Value), y = (Value/1000000))) +
    geom_bar(stat = "identity", color="black", fill= "lightsteelblue1", position=position_dodge()) +
      labs(title="Top 5 New Major Exports", y = "Vatu (millions)") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 35, vjust = 1.1, hjust=1, size = 5),
          axis.title.y = element_text(size = 9),
          axis.title.x = element_blank())
...
ggplot(data = totalMajorImports, aes(x = Year, y = value, group = factor(name_shortened))) +
  geom_line(aes(color = factor(name_shortened)), size = 1.5, alpha = 0.75) +
  labs(title="Principle Imports", y = "Vatu (millions)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 35, vjust = 1.1, hjust=1, size = 5),
        axis.title.y = element_text(size = 9),
        axis.title.x = element_blank(),
...
importsOther %>%
  ...
  ggplot(aes(x = reorder(Goods.Comm..Dsc., -total), y = (total/1000000))) +
    geom_bar(stat = "identity", color="black", fill= "lightsteelblue1", position=position_dodge()) +
  labs(title="Top 5 New Major Imports", y = "Vatu (millions)") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 35, vjust = 1.1, hjust=1, size = 5),
          axis.title.y = element_text(size = 9),
          axis.title.x = element_blank())
...
# Create a line graph
ggplot(historalAndCurrentMonthFood, aes(x = Year, y = (total/1000000), group = factor(Food.sub.category))) + 
  geom_line(aes(color = factor(Food.sub.category)), size = 1.5, alpha = 0.75) +
  labs(title="Imports of Dietary Risk Factors for Noncommunicable Diseases", y = "Vatu (millions)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 35, vjust = 1.1, hjust=1, size = 5),
        axis.title.y = element_text(size = 9),
        axis.title.x = element_blank(),
...
# Merge imports domestically produced classifications with cleaned data
importsSubstitutionChickenFish$HS.Code <- sapply(importsSubstitutionChickenFish$HS.Code, FUN=padWithZeros, "HS")
importSubstitutionChickenFishMergedWithClassifications <- merge(processedTradeStats, importsSubstitutionChickenFish, by="HS.Code", all.x=TRUE)

# Create data-frame for import substitutes for current month
chickenFishCommodities <- importSubstitutionChickenFishMergedWithClassifications[is.na(importSubstitutionChickenFishMergedWithClassifications$Livestock.Substitution) == FALSE, ]
chickenFishImportCommodities<- chickenFishCommodities[chickenFishCommodities$CP4 %in% c(4000, 4071, 7100), ]

# Calculate value of import substitutions for current month
chickenFishCategoryValue <- chickenFishImportCommodities %>%
  group_by(Year, Month, Livestock.Substitution) %>%
  summarise(total = sum(Stat..Value), .groups = "drop") %>%
  arrange(desc(total))

# Create data-frame for all import substitutions imports from historical data 
...
historicalChickenFishMergedWithClassifications <- merge(historicalImportsTenYears, importsSubstitutionChickenFish, by= "HS.Code", all.x=TRUE)
chickenFishCommodities <- historicalChickenFishMergedWithClassifications[is.na(historicalChickenFishMergedWithClassifications$Livestock.Substitution) == FALSE, ]

# Calculate value of Food Sub-Category for historical data
chickenFishCommodities$Stat..Value <- as.numeric(gsub(",", "", chickenFishCommodities$Stat..Value))
chickenFishHistoricalValue <- chickenFishCommodities %>%
  group_by(Year, Month, Livestock.Substitution) %>%
  summarise(total = sum(Stat..Value), .groups = "drop")
