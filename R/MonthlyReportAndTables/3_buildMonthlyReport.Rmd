...
tradeAgreementImportValue<- sum(tradeAgreementImports$Stat..Value)

# Calculate value of all exports that are destined to countries that have signed the MSG trade agreement 
tradeAgreementProxyExportValue <- sum(processedTradeStats[processedTradeStats$CP4 == 1000 & processedTradeStats$EXPORT.COUNTRY %in% c("FIJI", "NEW CALEDONIA", "PAPUA NEW GUINEA", "SOLOMON ISLANDS"), "Stat..Value"], na.rm=TRUE) #need to exclude re-exports from this calculation

# Proxy indicator calculated for NSDP indicator ECO 1.5.2 by combining imports and proxy exports
proxyIndictaorCalucluated1.5.2<- tradeAgreementImportValue + tradeAgreementProxyExportValue
...
indicator1.5.3Calculated<- balanceOfTradeByMajorPartnerCountries$Total
