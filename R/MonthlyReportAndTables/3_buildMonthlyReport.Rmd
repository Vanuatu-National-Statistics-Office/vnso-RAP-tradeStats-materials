...
processedTradeStats$Reg..Date <- as.Date(processedTradeStats$Reg..Date, format = "%Y-%m-%d")
processedTradeStats$Month <- format(processedTradeStats$Reg..Date, "%B")
...
# Read in the data from the baseline data (2016) for DARD <- define??
...
historicalImports$Month <- month.name[historicalImports$Month]

# Read in the imports substitution classification table
importsSubstitutionFile <- file.path(openDataFolder, "OPN_FINAL_ASY_FishChickenImportSubClassifications_31-01-20.csv")
...
ENV1.3.1 <- data.frame("Value of imports of food and products targeted by DARD"= importSubstitutionValue)
...
totalTradeAgreementValue<- importTradeAgreementValue + exportTradeAgreementValue

# Insert values into table
ECO1.5.2 <- data.frame("Trade by Trade Agreement Value"= totalTradeAgreementValue)
...
balanceTradeByMajorPartnerCountry <- totalMajorPartnerCountryExportValue - totalMajorPartnerCountryImportValue

# Insert values into table
ECO1.5.3 <- data.frame("Balance of Trade by Major Partner Countries"= balanceTradeByMajorPartnerCountry)
...
tradeAgreementGDPProportion<- (totalTradeAgreementValue/mostRecentGrossDomesticProduct)*100

# Insert values into table as a percentage to 2 decimal places
tradeAgreementGDPProportionRounded<- round(tradeAgreementGDPProportion, digits = 2)
ECO1.6.2<- data.frame("Value of Trade Agreement as Proportion of GDP"= tradeAgreementGDPProportionRounded)
...
majorExportCommoditiesValue<- majorExportCommodities$Total
ECO1.7.1 <- data.frame("Level of production of major commodities to include Cocoa and Copra"= majorExportCommoditiesValue)
...
# Insert values into table as a percentage to 2 decimal places
processedRawProportionRounded<- round(processedRawProportion$total, digits = 2)
ECO4.3.2<- data.frame("Ratio of Processed Export Commodities"= processedRawProportionRounded)
...
targetValueENV1.1.3<- ((importSubstitutionValue - baselineDARDImportsCurrentMonth)/baselineDARDImportsCurrentMonth)*100 # needs to be 15% to meet Target

# Insert values into table as a percentage to 2 decimal places
targetValueENV1.1.3Rounded<- round(targetValueENV1.1.3, digits = 2)
targetENV1.1.3<- data.frame("Percentage change in DARD targetted imports)"= targetValueENV1.1.3Rounded)
...
targetValueECO1.5.2<- ((exportTradeAgreementValue - baselineExportsTradeAgreementCurrentMonth)/baselineExportsTradeAgreementCurrentMonth)*100 # needs to be 15% to meet Target

# Insert values into table as a percentage to 2 decimal places
targetValueECO1.5.2Rounded<- round(targetValueECO1.5.2, digits = 2)
targetECO1.5.2 <- data.frame("Percentage Change in trade agreement exports"= targetValueECO1.5.2Rounded)
...
# Calculate ECO 1.5.3 Balance of Trade by major partner countries, Target 2020 exports increase by 20%
targetValueECO1.5.3<- ((totalMajorPartnerCountryExportValue - baselineMajorPartnerCountryExportValue)/baselineMajorPartnerCountryExportValue)*100 # needs to reach 20% Target

# Insert values into table as a percentage to 2 decimal places
targetValueECO1.5.3Rounded<- round(targetValueECO1.5.3, digits = 2)
targetECO1.5.3 <- data.frame("Percentage Change in Major Partner Country Exports"= targetValueECO1.5.3Rounded)
...
# Insert values into table as a percentage to 2 decimal places
targetECO1.6.2ValueRounded<- round(targetECO1.6.2Value, digits = 2)
targetECO1.6.2 <- data.frame("Percentage Change in total estimated value of trade agreement as proportion of GDP"= targetECO1.6.2ValueRounded)
...
targetECO1.7.1Value<- ((majorExportCommoditiesValue - baselineMajorProductionExports)/baselineMajorProductionExports)*100 # needs to reach 20% Target

# Insert values into table as a percentage to 2 decimal places
targetECO1.7.1ValueRounded<- round(targetECO1.7.1Value, digits = 2)
targetECO1.7.1 <- data.frame("Percentage Change in major commodity exports"= targetECO1.7.1ValueRounded)
...
targetECO4.3.2Value<- ((processedRawProportion - baselineProcessedRawValue)/baselineProcessedRawValue)*100 # needs to reach 20% Target

# Insert values into table as a percentage to 2 decimal places
targetECO4.3.2ValueRounded<- round(targetECO4.3.2Value$total, digits = 2)
targetECO4.3.2 <- data.frame("Percentage change in processed export commodities"= targetECO4.3.2ValueRounded)
...
countryRecords <- data.frame("ISO3" = sample(world$sov_a3, size = 50),
                             "RandomValue" = runif(50))

# Merge in my example data
world <- merge(world, countryRecords, by.x = "sov_a3", by.y = "ISO3", all.x = TRUE)
...
# Create the Trade by Trade Agreement table
tradeByTradeAgreement <- data.frame(
  ...
  "FIJI EXPORTS"=NA, "FIJI IMPORTS"=NA,"PAPUA NEW GUINEA EXPORTS"=NA, "PAPUA NEW GUINEA IMPORTS"=NA, "SOLOMON ISLANDS EXPORTS"=NA,"SOLOMON ISLANDS IMPORTS"=NA, "Total"=NA,
  check.names=FALSE, stringsAsFactors=FALSE)
...
majorExportsValue<- majorMonthlyExports %>%
  ...
  group_by(Year, Month, Copra, Coconut.Oil, Beef, Cocoa, Kava) 
...
# Create dataframe of current month major exports
copra<- sum(majorExportCommoditiesValues$Copra)
coconutOil<- sum(majorExportCommoditiesValues$CoconutOil)
beef<- sum(majorExportCommoditiesValues$Beef)
cocoa<- sum(majorExportCommoditiesValues$Cocoa)
kava<- sum(majorExportCommoditiesValues$Kava)
majorExportCurrentMonth <- data.frame("Year"= "2021", "Month"= "January", "Copra"= copra, "Coconut Oil"=coconutOil, "Beef"=beef, "Cocoa"=cocoa, "Kava"=kava)

# Add current month to historical data before visualising 
totalMajorExports<- rbind(majorExportsValue, majorExportCurrentMonth)
...
```{r health imports through time, fig.height=1.75}
...
historicalImportsTenYears$HS <- sapply(historicalImportsTenYears$HS, FUN=padWithZeros, "HS")
historicalHealthMergedWithClassifications <- merge(historicalImportsTenYears, productsRelatedHealth, by.x = "HS", by.y = "HS.Code", all.x=TRUE)
...
foodSubCategoryHistoricalValue <- unhealthlyHistoricalCommodities %>%
  group_by(Year, Month, Food.sub.category) %>%
  summarise(total = sum(Value_Sum), .groups = "drop")
...
# Create a line graph
ggplot(historalAndCurrentMonthFood, aes(x = Year, y = total, color = Food.sub.category)) + 
  geom_line()
...
nsdpIndicators$`Target Percentage (%)` <- round(nsdpIndicators$`Target Percentage (%)`, digits = 2)

# Add indicator direction
nsdpIndicators$`Indicator Value (Increased/Decreased)` <- ifelse(nsdpIndicators$`Target Percentage (%)` > 0, 
                                                                 "Increased",
                                                                 ifelse(nsdpIndicators$`Target Percentage (%)` < 0,
                                                                        "Decreased",
                                                                        NA))
...
```{r major partner country trade, warning=FALSE, fig.width=5.5, fig.height=2.4}
...
# Add current month to historical data before visualising - unless already there!
totalMajorExports <- majorExportsValue
if(majorExportCommoditiesValues$Year %in% majorExportsValue$Year == FALSE &&
   majorExportCommoditiesValues$Month %in% majorExportsValue$Month == FALSE ){
...
exportsOtherandNA %>%
  head(5) %>%
  ggplot(aes(x = reorder(Goods.Comm..Dsc., -total), y = total)) +
    geom_bar(stat = "identity") +
    ylab("Total export value") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 35, vjust = 1.1, hjust=1, size = 5),
...
# Add current month to historical data before visualising - unless already there!
totalMajorImports <- majorImportsValue
if(majorImportCommoditiesValues$Year %in% majorImportsValue$Year == FALSE &&
   majorImportCommoditiesValues$Month %in% majorImportsValue$Month == FALSE ){
...
# Create data-frame for all unhealthy imports from historical data 
historicalImportsTenYears <- historicalImports[historicalImports$Year %in% 2011:currentYear, ]
historicalImportsTenYears$HS <- sapply(historicalImportsTenYears$HS, FUN=padWithZeros, "HS")
historicalHealthMergedWithClassifications <- merge(historicalImportsTenYears, productsRelatedHealth, by.x = "HS", by.y = "HS.Code", all.x=TRUE)
unhealthlyHistoricalCommodities <- historicalHealthMergedWithClassifications[is.na(historicalHealthMergedWithClassifications$Unhealthy.Focus.Food.Category) == FALSE, ]
...
# Create a line graph
ggplot(historalAndCurrentMonthFood, aes(x = Year, y = total, group = factor(Food.sub.category))) + 
...
# TODO Divide monthly values by actual and estimated population data from the Census
...
# TODO Add code to generate data and plot current figure
...
## Methodology and meta-data
>>>>>>> ae3d37eb6de1460cc71e64d3932dd929126c3fec
