...
# Extract the balance of trade data and extract relevant statistics
balanceOfTradeFull <- read.xlsx(finalWorkbookFileName, sheet="1_BOT", skipEmptyRows=TRUE, check.names=FALSE)
annual <- balance

# Load the final tables workbook for editing
finalWorkbook <- loadWorkbook(finalWorkbookFileName)

# Extract the Annual t

# Insert a new row

# Add the year

# Add month and balance of trade statistics

# Save the edited workbook as a new file
editedFinalWorkbook <- gsub(pattern=".xlsx", replacement="_EDITED.xlsx", finalWorkbookFileName)
saveWorkbook(workBook, file=editedFinalWorkbook, overwrite=TRUE)
```
