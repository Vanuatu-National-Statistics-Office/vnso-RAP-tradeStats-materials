...
# Convert the Weight Net to numeric
tradeStatsNoDup$Wgt..Net <- as.numeric(tradeStats$Wgt..Net)
...
## Merge HS4 classifcations with cleaned data

```{r merge Mode of Transport classifcations with cleaned dataset}

# Merge HS4 classifications with cleaned data
tradeStatsFileMergeTransport <- file.path("..", "data", "open", "OPN_FINAL_ASY_HS4_31-01-20.xlsx") 
hsClassifications <- read.xlsx(tradeStatsFileMergeTransport, sheet=1, skipEmptyRows=TRUE)
tradeStatsCommoditiesMergedWithClassifications <- merge(tradeStatsCommodities, hsClassifications, by="HS.Code_4", all.x=TRUE)

# Check if any observations are missing
searchForMissingObservations(tradeStatsCommoditiesMergedWithClassifications, by="HS.Code_4", column="HS4_Commodities")
```
...
fileNameParts <- strsplit(processedTradeStats, split="_")[[1]]
...
principleExports <- unique(processedTradeStats[, "Principle.Exports"])
columnCategories[["Others products"]] <- principleExports[principleExports %in% unlist(columnCategories) == FALSE]
...
```{r calculate principle reExports statistics with base R, eval=FALSE, echo=FALSE}
...
# Define the categories used for each column
categoryColumn <- "Principle.ReExports"
columnCategories <- list(
  "Alcoholic Drinks"=c("Alcoholic Drinks"),
...
principleReExports <- unique(processedTradeStats[, "Principle.ReExports"])
columnCategories[["Others products"]] <- principleReExports[principleReExports %in% unlist(columnCategories) == FALSE]

# Build the table
principleReExports <- buildRawSummaryTable(processedTradeStats, codesCP4, categoryColumn, columnCategories)
prettyTable(principleReExports)


```

```{r calculate total principle exports statistics with base R, eval=FALSE, echo=FALSE}

# Total Exports and Re-Exports
columns <- unique(c(colnames(totalPrincipleExports), colnames(totalPrincipleReExports)))
totalPrincipleExportsandReExports <- data.frame(matrix(0, nrow=1, ncol=length(columns)))
colnames(totalPrincipleExportsandReExports) <- columns
totalPrincipleExportsandReExports[names(totalPrincipleExports)] <- totalPrincipleExports
totalPrincipleExportsandReExports[names(totalPrincipleReExports)] <- totalPrincipleExportsandReExports[names(totalPrincipleReExports)] + totalPrincipleReExports
...
prettyTable(totalPartnerCountryExports)
...
prettyTable(totalPartnerCountryImports)
...
prettyTable(totalPartnerRegionExports)
...
prettyTable(totalPartnerRegionImports)
...
# Calculate Trade by Trade Agreement (whether the commodity complies with the trade agreement can only be confirmed by the receiving country hence only for imports)


#Create subset of trade agreements separately then run the principle commodities 
...
## International National Merchandise Trade Report Highlights


## National Sustainable Development Plan Indicators- Table  

```{r calculate NSDP indicators and their Target value}

## ECO 1.4.1 Value and volume of national trade/merchandise trade (use Exported Trade as proxy), Target by 2030 increase 20%

totalExportsValue <- sum(processedTradeStats[processedTradeStats$CP4 %in% c(1000, 3071), "Stat..Value"], na.rm=TRUE)
totalExportsVolume <- sum(processedTradeStats[processedTradeStats$CP4 %in% c(1000, 3071), "Wgt..Net"], na.rm=TRUE)

# Insert values into table
exportValueVolume <- data.frame("Value"=totalExportsValue, "Volume"=totalExportsVolume)
prettyTable(exportValueVolume)

# Calculate percentage increase of export value and volume since 2016



## ECO 1.5.2 Trade by trade agreement (value), Target by 2020 exports increase 15% (we are only aware of imports, exports certified by receiving country, and we can give a proxy of exports of all commodities to MSG countries)

# Create proxy for exports that are aligned to Trade by Trade agreements (only include exports not re-exports as under trade agreement have to be produced within the country)
msgSubsetExports<- processedTradeStats[processedTradeStats$CP4 %in% c(1000), ]
msgProxyValueExports <- sum(msgSubsetExports[msgSubsetExports$EXPORT.COUNTRY %in% c("FIJI", "PAPUA NEW GUINEA", "SOLOMON ISLANDS", "NEW CALEDONIA"), "Stat..Value"], na.rm=TRUE)

# Imports aligned to MSG agreement
msgValueImports <- sum(processedTradeStats[processedTradeStats$PRF %in% c("MSG"), "Stat..Value"], na.rm=TRUE)

# Trade by Trade Agreement value approximation 
tradeByTradeAgreementValue<- msgProxyValueExports + msgValueImports 

# Insert values into table
dataframeTradebyTradeAgreement <- data.frame("Trade by Trade Agreement Value"=tradeByTradeAgreementValue)
prettyTable(dataframeTradebyTradeAgreement)

# Calculate percentage increase of export and import value aligned to Trade Agreements since 2016




# ECO 1.5.3 Balance of trade by major partner countries, Target by 2020 exports increase 20%

# Use calculation from previous table and extract value
balanceOfTradeMajorPartnerCountries<- balanceOfTradeByMajorPartnerCountries[ , 22]

# Insert values into table
dataframeBalanceOfTradeMajorPartnerCountries <- data.frame("Balance of Trade Major Partner Countries"=balanceOfTradeMajorPartnerCountries)
prettyTable(dataframeBalanceOfTradeMajorPartnerCountries)

# Calculate percentage increase of exports



## ECO 1.6.2 Total estimated value of trade agreement as proportion of GDP, Target by 2030 increase 15%


## ECO 1.7.1 Level of production of major commodities to include cocoa and copra, Target by 2030 increase 10-15%
...
# Build the table
productionOfMajorCommodities <- buildRawSummaryTable(processedTradeStats, codesCP4, categoryColumn, columnCategories)
prettyTable(productionOfMajorCommodities)

# Calculate percentage increase of production of major exports


# `ECO4.3.2 Ratio of processed export commodities (including coconut, kava, cocoa, coffee) to raw exports, Target by 2020 increase 15%


```

```{r insert NSDP indicators into final formatted table}


```

```{r insert automated text that states the growth rate needed for the policy objectives to meet their target indicators}


```

## Trade Balance by major partner countries- Dumbbell Chart

```{r visualise trade balance by major partner countries}

# Create Dumbbell chart to highlight balance of trade by exports and imports (both data-frames previously calculated)

# Order Exports of major partner countries
orderPartnerCountryExports<- totalPartnerCountryExports[, order(totalPartnerCountryExports)]
dataframeOrderPartnerCountryExports <- data.frame("Partner Country Exports"=orderPartnerCountryExports)
prettyTable(dataframeOrderPartnerCountryExports)

# Order Imports of major partner countries
orderPartnerCountryImports<- totalPartnerCountryImports[, order(totalPartnerCountryImports)]
dataframeOrderPartnerCountryImports <- data.frame("Partner Country Exports"=orderPartnerCountryImports)
prettyTable(dataframeOrderPartnerCountryImports)


```

```{r insert automated text that states the Trade balances between major countries, countries we did not export to from major partner countries}


```

## Trade Balance of Major Pacific Islands (excluding Fiji & New )- Bar Chart

```{r visualise trade balance by pacific countries}
...
prettyTable(totalPacificCountryExports)
...
prettyTable(totalPacificCountryImports)
...
prettyTable(tradeBalancePacificIslands)

```

```{r insert automated text highlighting trade balance; difference between value of imports and exports comparing this month to same month in previous year}


```

## Trade Balance of Emerging Markets (excluding Major Partner Countries)- Bar Chart (issue here is have to calculate top again, excluding the major-ask Joe)

```{r visualise trade balance by Emerging Markets}
